---
title: OpenClaw x402-Layer Skill
...
<Note>
  Current release: **v1.10.3**. This release extends OWS support to wallet-auth list/support flows while keeping deep ERC-8004 registration and on-chain update transaction paths on direct signing keys.
</Note>
...
```bash
npx clawhub@latest install x402-layer
pnpm dlx clawhub@latest install x402-layer
bunx clawhub@latest install x402-layer
```

ClawHub listing:

- `https://clawhub.ai/ivaavimusic/x402-layer`
...
- OpenClaw-native `x402-layer` skill distribution
- ClawHub install path
- self-hosted worker install path
- full script set for payments, endpoints, marketplace, webhooks, support, and agent identity
