...
<Note>
  Current release: **v1.10.0** — adds **Robinhood Chain / USDG** as an x402 payment rail alongside **SGL Grid** (OpenAI-compatible confidential inference) and the **Provide Compute / run-a-node** module. x402 Base/Solana/MegaETH/Robinhood payments are live. Pre-loaded USD credits available for payment-free provisioning, extending, and Grid inference. MegaETH uses USDm (ERC-2612 permit) and Robinhood Chain uses USDG (EIP-3009 `transferWithAuthorization`), both with an embedded facilitator — gasless for users. MPP Tempo USDC.e payments are also live for agent/CLI provisioning.
</Note>
