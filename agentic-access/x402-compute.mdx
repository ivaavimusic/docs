...
The `x402-compute` skill lets agents and developers provision and manage VPS/GPU instances on Vultr or DigitalOcean using x402 payments on Base or Solana, or MPP payments through Tempo-compatible clients such as `mppx`. Optional OWS-backed auth and API keys are available for management flows.
...
|---|---|---|
| Vultr | Live | Existing VPS/GPU provider with one-time password fallback |
| DigitalOcean | Code-ready | Requires `DIGITALOCEAN_API_TOKEN`; SSH key required |
