...
The Singularity MCP server exposes **35 tools** that let any MCP-compatible AI client (Claude Desktop, Cursor, Windsurf, etc.) interact with the x402Studio marketplace. Browse listings, manage endpoints, run payment flows, and register agents — all through natural language.
...
| **Legacy Endpoint Key** | `x402_{32hex}` | Single endpoint only |
...
| Tool | Description |
|------|-------------|
| `browse_marketplace` | Search/filter listings by type, category, chain, rating, with pagination |
| `get_listing` | Get full details for a specific listing by slug |
| `get_featured` | Get featured marketplace items |
| `get_top_rated` | Get top-rated listings |
| `get_agent` | Get ERC-8004 agent info by network and ID |
| `list_categories` | List all available marketplace categories |
| `list_networks` | List supported blockchain networks |
| `list_agents` | List all registered ERC-8004 agents with pagination |

### Example: Browse AI Endpoints
...
## Phase 2: Management (PAT Required)

Manage your x402Studio resources programmatically.

| Tool | Description | Scope |
|------|-------------|-------|
| `list_my_endpoints` | List all endpoints in owner scope | `mcp:read` |
...
| `list_my_products` | List all products in owner scope | `mcp:read` |
| `update_product` | Update metadata, pricing, branding, listing state | `mcp:products:write` |
| `set_webhook` | Set/update webhook URL, returns signing secret | `mcp:endpoints:write` |
| `remove_webhook` | Remove webhook from endpoint | `mcp:endpoints:write` |

### Example: Update Endpoint Price
...
```json
{
  ...
  "arguments": {
    "slug": "my-ai-api",
    "accessToken": "sgl_pat_your_token",
    "price": "0.05"
  }
}
```
...
| Tool | Description |
|------|-------------|
| `request_endpoint_creation_payment` | Get the x402 challenge to create a new agent endpoint |
...
4. **Done** — The resource is created/topped up/purchased
...
- PATs and endpoint keys are passed per-request and **never stored** by the MCP server
- Read-only PATs (`mcp:read`) cannot perform writes
- Endpoint-write PATs cannot mutate products (and vice versa)
- Revoked or expired PATs are rejected immediately
- All tool calls are scoped to the authenticated owner's resources
...
| Registry | `io.github.ivaavimusic/singularity` |
| Version | `1.6.0` |
