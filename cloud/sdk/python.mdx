...
Send an image to a **vision model** (e.g. `qwen2.5-vl-3b`) by giving a message a list of
...
returns the full OpenAI-style response. See the [Embeddings guide](/cloud/grid/embeddings)
...
# Matryoshka: shorter vectors to save storage
short = grid.embed("nomic-embed-text-v1.5", "cat", dimensions=256)

# Full OpenAI-style response (usage, indices)
resp = grid.embeddings("bge-small-en-v1.5", ["a", "b"])
print(resp["usage"])
```
