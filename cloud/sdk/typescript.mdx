...
Send an image to a **vision model** (e.g. `qwen2.5-vl-3b`) by giving a message an array of
content parts — a `text` part and an `image_url` part (a data URL or an https URL). The image
is sealed end-to-end with the rest of your prompt; only the image count is disclosed so the
grid can quote it and route to a vision-capable node. Image requests are non-streaming.

```typescript
import { GridClient, type ChatContentPart } from "@singularity-layer/grid";
