...
For agentic creation, prefer Singularity MCP with a scoped dashboard PAT. The owner-scoped MCP tools are `list_my_campaigns`, `create_campaign`, and `update_campaign`. Endpoint API keys are intentionally not accepted for campaign creation.
